...
class Seq2SeqModel:
    def __init__(
        ...
        **kwargs
        ...
        if encoder_decoder_type in ["bart", "bart2decoder", "marian", "blender", "blender-large"]:
            self.model = model_class.from_pretrained(encoder_decoder_name)
            if encoder_decoder_type in ["bart", "bart2decoder", "blender", "blender-large"]:
    ...
    def train(
        ...
        # add MLP optimizer for sharpness
        parameters_MLP = list(self.fc.named_parameters())
        without = []
        no_decay = []
        parameters_MLP = [
            {'params': [p for n, p in parameters_MLP 
                if not (any(nd in n for nd in no_decay) or any(wo in n for wo in without))], 'weight_decay': 0.0}
        ]
        optimizer_MLP = AdamW(parameters_MLP, lr=args.learning_rate_MLP, eps=args.adam_epsilon)
        ...
        for current_epoch in train_iterator:
            ...
            for step, batch in enumerate(batch_iterator):
                ...
                ##### MLP loss
                decoder_output_last_hidden_states = outputs.decoder_hidden_states[-1]
                decoder_entity_last_words_index = (inputs['decoder_input_ids']==16).nonzero()[:, 1]
                decoder_entity_last_hidden_state = torch.index_select(decoder_output_last_hidden_states, 1, decoder_entity_last_words_index)
                # decoder_output_last_hidden_states[:, decoder_entity_last_words_index,:]
                for ii, decoder_entity_last_hidden_state_line in enumerate(decoder_entity_last_hidden_state):
                    if ii == 0:
                        final_decoder_entity_last_hidden_state = decoder_entity_last_hidden_state_line[ii].view(1,-1)
                    else:
                        final_decoder_entity_last_hidden_state = torch.cat((final_decoder_entity_last_hidden_state, decoder_entity_last_hidden_state_line[ii].view(1,-1)),dim=0)
                logits_MLP = F.softmax(self.fc(final_decoder_entity_last_hidden_state))
                labels_MLP = []
                for l_line in inputs['decoder_input_ids']:
                    if 1440 in l_line:
                        labels_MLP.append(0)
                    else:
                        labels_MLP.append(1)
                labels_MLP = torch.tensor(labels_MLP).to(self.device)
                loss_MLP = self.loss_func_MLP(logits_MLP, labels_MLP)

                # MLP acc
                true_preds = (torch.max(logits_MLP, 1)[1] == labels_MLP).float()
                true_preds_count = true_preds.sum().item()
                current_acc_MLP = true_preds_count / logits_MLP.shape[0]
                all_MLP_pred.append((true_preds.cpu().numpy(), labels_MLP.cpu().numpy()))
                MLP_true_count += true_preds_count
                all_entites_count += logits_MLP.shape[0]
                all_MLP_acc = MLP_true_count / all_entites_count
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    loss_MLP.backward()
                    torch.nn.utils.clip_grad_norm_(self.fc.parameters(), args.max_grad_norm)
                    optimizer_MLP.step()
                    self.fc.zero_grad()
    ...
    def evaluate(self, eval_dataset, output_dir, verbose=True, silent=False, **kwargs):
        ...
        nb_eval_steps = 0
        eval_all_MLP_pred = []
        eval_MLP_true_count = 0
        eval_all_entites_count = 0
        ...
        for batch in tqdm(eval_dataloader, disable=args.silent or silent, desc="Running Evaluation"):
            ...
            with torch.no_grad():
                ...
                labels_MLP = torch.tensor(labels_MLP).to(self.device)
                loss_MLP = self.loss_func_MLP(logits_MLP, labels_MLP)
                eval_loss_MLP += loss_MLP.item()
                # MLP acc
                true_preds = (torch.max(logits_MLP, 1)[1] == labels_MLP).float()
                true_preds_count = true_preds.sum().item()
                # current_acc_MLP = true_preds_count / logits_MLP.shape[0]
                eval_all_MLP_pred.append((true_preds.cpu().numpy(), labels_MLP.cpu().numpy()))
                eval_MLP_true_count += true_preds_count
                eval_all_entites_count += logits_MLP.shape[0]
        ...
        results["eval_loss_MLP"] = eval_loss_MLP
        results["eval_all_MLP_acc"] = eval_all_MLP_acc
    ...
    def load_and_cache_examples(self, data, evaluate=False, no_cache=False, verbose=True, silent=False):
        ...
        if args.dataset_class:
            ...
            if args.model_type in ["bart", "bart2decoder", "marian", "blender", "blender-large"]:
    ...
    def _save_model(self, output_dir=None, optimizer=None, scheduler=None, model=None, results=None):
        ...
        if model and not self.args.no_save:
            torch.save({'state_dict': self.fc.state_dict()}, os.path.join(output_dir, "MLP.ckpt"))
